...
exports.renderSignup = function(req, res, next) {
	...
	if (!req.user) {
		...
		res.render('signin', {
			...
			badmessage: ('error') //passes the error stored in flash
		});
	} else {
		return res.redirect('/');
	}
	...
	if (!req.user) {
		...
		user.save((err) => {
			...
			if (err) {
				...
				// ('error', message); //save the error into flash memory
...
exports.displayAllStudents = (req,res) => {
    console.log("displayAllStudents")
    var students = User.find({}, (err, studentsList) => {
            console.log(err)
